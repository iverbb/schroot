...
namespace schroot
{
  ...
  template<>
  error<bin::schroot::main::error_code>::map_type
  error<bin::schroot::main::error_code>::error_strings =
    {
      // TRANSLATORS: %4% = file
      {bin::schroot::main::CHROOT_FILE,       N_("No chroots are defined in ‘%4%’")},
      // TRANSLATORS: %4% = file
      // TRANSLATORS: %5% = file
      {bin::schroot::main::CHROOT_FILE2,      N_("No chroots are defined in ‘%4%’ or ‘%5%’")},
      // TRANSLATORS: %1% = file
      {bin::schroot::main::CHROOT_NOTDEFINED, N_("The specified chroots are not defined in ‘%1%’")},
      {bin::schroot::main::SESSION_INVALID,   N_("%1%: Invalid session name")}
    };
...
namespace bin
{
  namespace schroot
  {
    ...
    main::main (schroot::options::ptr& options):
      bin::common::main("schroot",
    ...
    main::action_version (std::ostream& stream)
    {
      ...
      stream << '\n'
             ...
             << '\n';
      ::schroot::chroot::facet::factory::print_facets(stream);
    ...
    main::action_info ()
    {
      for(auto chroot_name = this->chroot_names.begin();
          ...
          ++chroot_name)
        {
          ...
          ::schroot::chroot::config::chroot_map::const_iterator c = this->chroots.find(*chroot_name);
          assert(c->second);
    ...
    main::action_config ()
    {
      std::cout << "# "
        ...
        % this->program_name % VERSION % ::schroot::date(time(0))
                << endl;
      std::cout << endl;

      ::schroot::keyfile info;
      ...
      std::cout << ::schroot::keyfile_writer(info) << std::flush;
    }
    ...
    main::get_chroot_options ()
    {
      ...
      if (this->opts->all_chroots ||
          ...
          this->opts->all_source_chroots)
        {
          if (this->opts->all_chroots)
            {
              ...
              if (this->opts->action == options::ACTION_LIST &&
                  !this->opts->exclude_aliases)
          ...
          if (this->opts->all_sessions)
            {
              ::schroot::string_list sessions;
              if (this->opts->action == options::ACTION_LIST &&
                  !this->opts->exclude_aliases)
          ...
          if (this->opts->all_source_chroots)
            {
              ::schroot::string_list sources;
              if (this->opts->action == options::ACTION_LIST &&
                  !this->opts->exclude_aliases)
      ...
      else
        {
          ...
          if (this->opts->action == options::ACTION_SESSION_RECOVER ||
              this->opts->action == options::ACTION_SESSION_RUN ||
              this->opts->action == options::ACTION_SESSION_END)
            chroot_namespace = "session";

          // Validate and normalise
          this->chroot_names = this->opts->chroots;
    ...
    main::run_impl ()
    {
      ...
      if (this->opts->action == options::ACTION_VERSION)
        {
      ...
      for(const auto& chroot_name : this->chroot_names)
        {
          ...
          const auto chroot = this->chroots.find(chroot_name);
          assert(chroot->second);
          ::schroot::session::chroot_list::value_type e;
      ...
      if (this->config->find_alias("session", this->opts->session_name))
        throw error(this->opts->session_name, SESSION_INVALID);

      /* Print chroot information for specified chroots. */
      if (this->opts->action == options::ACTION_INFO)
        {
          action_info();
          return EXIT_SUCCESS;
        }
      if (this->opts->action == options::ACTION_LOCATION)
        {
          action_location();
          return EXIT_SUCCESS;
        }
      if (this->opts->action == options::ACTION_CONFIG)
        {
          action_config();
          return EXIT_SUCCESS;
        }
      ...
      /* Create a session. */
      ::schroot::session::operation sess_op(::schroot::session::OPERATION_AUTOMATIC);
      if (this->opts->action == options::ACTION_SESSION_BEGIN)
        sess_op = ::schroot::session::OPERATION_BEGIN;
      else if (this->opts->action == options::ACTION_SESSION_RECOVER)
        sess_op = ::schroot::session::OPERATION_RECOVER;
      else if (this->opts->action == options::ACTION_SESSION_RUN)
        sess_op = ::schroot::session::OPERATION_RUN;
      else if (this->opts->action == options::ACTION_SESSION_END)
        sess_op = ::schroot::session::OPERATION_END;
      ...
      try
        {
          ...
          if (!this->opts->command.empty())
            this->session->get_auth()->set_command(this->opts->command);
          if (!this->opts->directory.empty())
            this->session->get_auth()->set_wd(this->opts->directory);
          if (!this->opts->shell.empty())
            this->session->set_shell_override(this->opts->shell);
          this->session->set_preserve_environment(this->opts->preserve);
          this->session->set_session_id(this->opts->session_name);
          this->session->set_force(this->opts->session_force);
          if (this->opts->quiet)
            this->session->set_verbosity("quiet");
          else if (this->opts->verbose)
            this->session->set_verbosity("verbose");
          this->session->set_user_options(this->opts->useroptions_map);
    ...
    main::add_session_auth ()
    {
...
#ifdef SCHROOT_FEATURE_PAM
      ::schroot::auth::auth::ptr auth = ::schroot::auth::pam::create("schroot");

      ::schroot::auth::pam_conv::auth_ptr pam_auth =
        std::dynamic_pointer_cast<::schroot::auth::pam>(auth);

      ::schroot::auth::pam_conv::ptr conv = ::schroot::auth::pam_conv_tty::create(pam_auth);
      ...
      if (!this->opts->user.empty())
        this->session->get_auth()->set_user(this->opts->user);
    }
